...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print max, min and mean of arrays
        #print(f"Max: {torch.max(arrays).item()}, Min: {torch.min(arrays).item()}, Mean: {torch.mean(arrays).item()} "   )
...
def setup_training(encoderunet, unetdecoder, resume=0):
    ...
    if resume == 1:
        # Load checkpoint
        checkpoint = torch.load('Cross_CP/Cross_VMAT_Artifical_data_1500_01Dec_amp_parallel_coll0_embedded_64_impLoss_NormDS_local_checkpoint.pth', 
                              map_location='cpu')
...
def train_cross(encoderunet, unetdecoder, train_loaders, val_loaders, device, batch_size, resume=0):
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        if rank == 0:
            ...
            torch.save(checkpoint, 'Cross_CP/Cross_VMAT_Artifical_data_1500_01Dec_amp_parallel_coll0_embedded_64_impLoss_NormDS_local_checkpoint.pth')
