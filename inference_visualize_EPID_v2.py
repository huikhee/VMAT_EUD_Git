"""Inference + visualization for the EPID v2 encoder/decoder model.
...
v2 differs from v1 in that the models use attention/transformers to replace
flatten+FC blocks, but tensor I/O shapes remain the same.
"""
...
from generate_and_train_EPID_v2 import (
    CustomDataset,
    EncoderUNet,
    UNetDecoder,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Inference + visualization for EPID v2")
    ...
    parser.add_argument("--checkpoint", type=str, default="Cross_CP/EPID_v2_16Dec25_checkpoint.pth")
    ...
    parser.add_argument("--output-dir", type=str, default="viz_outputs_v2", help="Directory for PNGs")
    ...
    for ds_idx, ds in datasets.items():
        ...
        for si in chosen:
            ...
            title = f"EPID v2 | Dataset {ds_idx} | Sample {si}"
            visualize(
                epid_gt,
                epid_img,
                v1_gt,
