"""Inference + visualization for the EPID v1 encoder/decoder model.

Loads the saved encoder/decoder checkpoint from training, samples data from
stored datasets, runs forward passes, and saves comparison plots of predicted
versus target EPID frames and leaf/jaw positions.
"""
...
from generate_and_train_EPID_v1 import (
    CustomDataset,
    EncoderUNet,
    UNetDecoder,
...
def parse_range(spec: str) -> Iterable[int]:
    """Parse a start:end[:step] string into ints."""
    parts = spec.split(":")
...
def strip_module_prefix(state_dict: dict) -> dict:
    """Remove leading 'module.' for DDP checkpoints."""
...
def compute_gamma(epid_gt: np.ndarray, epid_pred: np.ndarray) -> Tuple[np.ndarray | None, float | None]:
    ...
    Notes:
    - EPID frames are treated as having an in-plane extent of ~[-130, 130] mm in both axes
      (consistent with other plotting in this repo), giving ~2 mm per pixel for 131×131.
    - Uses global gamma with 3%/3mm and default lower dose cutoff (20%).
    """
    try:
...
def plot_losses(train_losses, val_losses, out_path: Path) -> None:
    """Plot train/val losses if available."""
    ...
    # Use log scale; shift if any non-positive values
    min_val = None
...
# colormap for gamma (green pass, red fail) placeholder
COLORS = ["green", "red"]
CMAP = mcolors.ListedColormap(COLORS)
NORM = mcolors.BoundaryNorm([0, 1, 2], len(COLORS), clip=True)


def visualize(epid_gt: np.ndarray, epid_pred: np.ndarray,
              v1_gt: np.ndarray, v2_gt: np.ndarray,
              v1_pred: np.ndarray, v2_pred: np.ndarray,
              scalars_gt: np.ndarray, scalars_pred: np.ndarray,
              v1_weight: np.ndarray, v2_weight: np.ndarray,
    ...
    v1_prev_t = v1_gt_np[:52]; v1_curr_t = v1_gt_np[52:]
    v2_prev_t = v2_gt_np[:52]; v2_curr_t = v2_gt_np[52:]
    v1_prev_p = v1_pred_np[:52]; v1_curr_p = v1_pred_np[52:]
    v2_prev_p = v2_pred_np[:52]; v2_curr_p = v2_pred_np[52:]
    ...
    plot_leaf_panel(axes[1, 0], v1_prev_t, v1_prev_p, v2_prev_t, v2_prev_p,
                    jaw_low_prev_t, jaw_low_prev_p, jaw_up_prev_t, jaw_up_prev_p, "Prev CP v1/v2 + jaws")
    plot_leaf_panel(axes[1, 1], v1_curr_t, v1_curr_p, v2_curr_t, v2_curr_p,
                    jaw_low_curr_t, jaw_low_curr_p, jaw_up_curr_t, jaw_up_curr_p, "Curr CP v1/v2 + jaws")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Inference + visualization for EPID v1")
    parser.add_argument("--dataset-range", type=str, default="0:1", help="start:end[:step] of datasets")
    parser.add_argument("--samples-per-dataset", type=int, dest="samples_per_dataset", default=2,
                        help="Random samples per dataset")
    parser.add_argument("--samples_per_dataset", type=int, dest="samples_per_dataset",
                        help="Alias for --samples-per-dataset")
    parser.add_argument("--max-total-samples", type=int, default=None, help="Optional global cap")
    parser.add_argument("--checkpoint", type=str, default="Cross_CP/EPID_v1_15Dec25_checkpoint.pth")
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--regenerate-missing", action="store_true", dest="regenerate_missing",
                        help="Regenerate dataset if not found (needs KM)")
    parser.add_argument("--output-dir", type=str, default="viz_outputs", help="Directory for PNGs")
    ...
    if args.regenerate_missing:
        ...
        import scipy.io  # local import to avoid hard dependency when not used
    ...
    for ds_idx, ds in datasets.items():
        ...
        def ensure_nchw(t: torch.Tensor) -> torch.Tensor:
            t = t.squeeze()
            if t.dim() == 2:  # H, W
                ...
                if t.shape[0] == 1:
                    ...
                    # Assume already (C,H,W); prepend batch dim
                    t = t.unsqueeze(0)
            return t
        ...
        for si in chosen:
            ...
            arrays = ensure_nchw(arrays).to(device)
            arrays_p = ensure_nchw(arrays_p).to(device)
            ...
            title = f"Dataset {ds_idx} | Sample {si}"
            visualize(
                epid_gt,
                epid_img,
                v1_gt,
