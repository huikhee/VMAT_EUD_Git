"""Inference + visualization for the EPID v7 encoder/decoder model.

v7 changes (relative to v6) are primarily in the EPID geometry / image IO:
- EPID images are 256x256 at 1 mm pitch (FOV [-128, 128) mm)
- Dataset folder is VMAT_Art_data_256 (to avoid mixing resolutions)

Model architecture is otherwise consistent with the v6 forward/backward setup.
"""
...
from generate_and_train_EPID_v7 import (
    CustomDataset,
    EncoderUNet,
    UNetDecoder,
...
def compute_gamma(epid_gt: np.ndarray, epid_pred: np.ndarray) -> Tuple[np.ndarray | None, float | None]:
    ...
    # v7: 256x256 at 1 mm pitch, FOV [-128, 128) mm (pixel centers)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Inference + visualization for EPID v7")
    ...
    parser.add_argument("--checkpoint", type=str, default="Cross_CP/EPID_v7_20Dec25_checkpoint.pth")
    ...
    parser.add_argument("--output-dir", type=str, default="viz_outputs_v7", help="Directory for PNGs")
    ...
    for ds_idx, ds in datasets.items():
        ...
        for si in chosen:
            ...
            title = f"EPID v7 | Dataset {ds_idx} | Sample {si}"
            visualize(
                epid_gt,
                epid_img,
                v1_gt,
