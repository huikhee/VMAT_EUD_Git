...
def train_cross(encoderunet, unetdecoder, train_loaders, val_loaders, device, batch_size, resume=0):
    ...
    # Lightweight timing to diagnose GPU stalls on fast GPUs (e.g., H100).
    # Prints are rate-limited and only emitted on rank 0.
    TIMING_EVERY = int(os.environ.get('EPID_TIMING_EVERY', '50'))  # print every N batches
    TIMING_MAX_PRINTS_PER_EPOCH = int(os.environ.get('EPID_TIMING_MAX', '20'))
            ...
            for i, val_loader in enumerate(val_loaders):
                ...
                for batch_idx, (v1, v2, scalars, v1_weight, v2_weight, arrays, arrays_p) in enumerate(val_loader):
                    do_timing = (
                        ...
                        and (batch_idx % TIMING_EVERY == 0)
                        and (timing_prints < TIMING_MAX_PRINTS_PER_EPOCH)
                        and (i == 0)  # keep val timing minimal
                    )
