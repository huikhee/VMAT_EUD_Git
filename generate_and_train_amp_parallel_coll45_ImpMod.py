...
def save_dataset(dataset, dataset_num):
    ...
    os.makedirs("../Encoder_UNet_Decoder_Git/VMAT_Art_data", exist_ok=True)
    filename = os.path.join("../Encoder_UNet_Decoder_Git/VMAT_Art_data", f"Art_dataset_coll45_{dataset_num}.pt")
...
class ExtEncoder(nn.Module):
    ...
    def forward(self, vector1, vector2, scalars):
        ...
        vectors_encoded = F.leaky_relu(self.vector_fc(vectors_combined), 0.01)

        # Process each scalar individually
        scalars_encoded = [F.leaky_relu(fc(scalar), 0.01) for fc, scalar in zip(self.scalar_fc, scalars.unbind(dim=2))]
        ...
        latent_image = F.leaky_relu(combined_output, 0.01)
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.leaky_relu1 = nn.LeakyReLU(0.01, inplace=False)
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1)
        self.leaky_relu2 = nn.LeakyReLU(0.01, inplace=False)
...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels, resize_out, freeze_encoder):
        ...
        self.final_ReLU = nn.LeakyReLU(0.01, inplace=False)
...
def initialize_weights(model):
    ...
    for m in model.modules():
        if isinstance(m, nn.Conv2d):
            # Conv layers in UNet blocks - using He initialization for LeakyReLU
...
def setup_training(encoderunet, unetdecoder, resume=0):
    ...
    if resume == 1:
        # Load checkpoint
        checkpoint = torch.load('Cross_CP/Cross_VMAT_Artifical_data_1500_01Dec_amp_parallel_coll45_ImpMod_checkpoint.pth', 
                              map_location='cpu')
...
def train_cross(encoderunet, unetdecoder, train_loaders, val_loaders, device, batch_size, resume=0):
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        if rank == 0:
            ...
            torch.save(checkpoint, 'Cross_CP/Cross_VMAT_Artifical_data_1500_01Dec_amp_parallel_coll45_ImpMod_checkpoint.pth')
