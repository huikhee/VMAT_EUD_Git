"""Inference + visualization for the EPID v4 encoder/decoder model.
...
v4 differs from v3 in the forward-path tokenization/aggregation:
- Dedicated CLS token is used for pooling (instead of pooling the scalar token)
- Scalars are split into MU token + 2 jaw tokens
- Leaves are reorganized into 104 tokens (52 from v1 prev/curr, 52 from v2 prev/curr)

Tensor I/O shapes for training/inference remain compatible.
"""
...
from generate_and_train_EPID_v4 import (
    CustomDataset,
    EncoderUNet,
    UNetDecoder,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Inference + visualization for EPID v4")
    parser.add_argument("--dataset-range", type=str, default="0:1", help="start:end[:step] of datasets")
    parser.add_argument(
        "--dataset_range",
        type=str,
        dest="dataset_range",
        help="Alias for --dataset-range",
    )
    parser.add_argument(
        "--samples-per-dataset",
        type=int,
    ...
    parser.add_argument("--checkpoint", type=str, default="Cross_CP/EPID_v4_17Dec25_checkpoint.pth")
    ...
    parser.add_argument("--output-dir", type=str, default="viz_outputs_v4", help="Directory for PNGs")
    ...
    for ds_idx, ds in datasets.items():
        ...
        for si in chosen:
            ...
            title = f"EPID v4 | Dataset {ds_idx} | Sample {si}"
            visualize(
                epid_gt,
                epid_img,
                v1_gt,
