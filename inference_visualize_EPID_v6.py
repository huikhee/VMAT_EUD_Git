"""Inference + visualization for the EPID v6 encoder/decoder model.

v6 changes (relative to v5) are in the *backward* path (UNetDecoder):
- Multi-scale transformer memory tokens from the latent map (64/32/16 scales)
- Separate prev/curr EPID encoders with late fusion
...
from generate_and_train_EPID_v6 import (
    CustomDataset,
    EncoderUNet,
    UNetDecoder,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Inference + visualization for EPID v6")
    ...
    parser.add_argument("--checkpoint", type=str, default="Cross_CP/EPID_v6_17Dec25_checkpoint.pth")
    ...
    parser.add_argument("--output-dir", type=str, default="viz_outputs_v6", help="Directory for PNGs")
    ...
    encoderunet.eval()
    unetdecoder.eval()
    print(f"[INFO] Loaded checkpoint {ckpt_path} (mtime: {ckpt_mtime})")
    ...
    if train_losses or val_losses:
        ...
        # If the checkpoint doesn't have loss history, ensure we don't leave an old plot behind.
        if losses_png.exists():
            try:
                losses_png.unlink()
    ...
    for ds_idx, ds in datasets.items():
        ...
        for si in chosen:
            ...
            title = f"EPID v6 | Dataset {ds_idx} | Sample {si}"
            visualize(
                epid_gt,
                epid_img,
                v1_gt,
