...
class UNetDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        vector1 = self.vector_fc1(out)   # (B, vector_dim)
        vector2 = self.vector_fc2(out)   # (B, vector_dim)
...
def setup_training(encoderunet, unetdecoder, resume=0):
    ...
    if resume == 1:
        # Load checkpoint
        checkpoint = torch.load('Cross_CP/Cross_VMAT_Artifical_data_1500_01Dec_amp_parallel_coll0_embedded_64_local_checkpoint.pth', 
                              map_location='cpu')
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        for i, train_loader in enumerate(train_loaders):
            ...
            for batch_idx, (v1, v2, scalars, v1_weight, v2_weight, arrays, arrays_p) in enumerate(train_loader):
                ...
                print(f"Epoch [{epoch+1}/{EPOCHS}] Loader [{i+1}/{len(train_loaders)}] "
                      f"Batch [{batch_idx+1}/{len(train_loader)}]  "
                      f"mse_loss_v1: {mse_loss_v1.item():.2e} mse_loss_v2: {mse_loss_v2.item():.2e} "
                      f"mse_loss_scalars: {mse_loss_scalars.item():.2e} penalty_loss: {penalty_loss.item():.2e} "
                      f"mse_loss_v1_2: {mse_loss_v1_2.item():.2e} mse_loss_v2_2: {mse_loss_v2_2.item():.2e} "
                      f"mse_loss_scalars_2: {mse_loss_scalars_2.item():.2e} penalty_loss_2: {penalty_loss_2.item():.2e} ".ljust(500), end='\r')
        ...
        if rank == 0:
            ...
            torch.save(checkpoint, 'Cross_CP/Cross_VMAT_Artifical_data_1500_01Dec_amp_parallel_coll0_embedded_64_local_checkpoint.pth')
...
def train_ddp(rank, world_size, generate_flag, KM):
    ...
    for dataset_num in range(0, 561, 80):
