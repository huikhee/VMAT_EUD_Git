"""Inference + visualization for the EPID v9 encoder/decoder model.

v9 changes (relative to v8) include using a newly generated KM kernel during
synthetic dataset generation. The model architecture and IO are expected to
remain compatible with the v8 inference/visualization flow.

Notes on dataset loading:
- Some datasets may have been saved via torch.save() when CustomDataset lived in
  the script's __main__ module, which can break torch.load() in other contexts.
  This script includes a robust loader that can handle that case.
"""
...
import generate_and_train_EPID_v9 as v9
...
def compute_gamma(epid_gt: np.ndarray, epid_pred: np.ndarray) -> Tuple[np.ndarray | None, float | None]:
    ...
    # EPID is 256x256 at 1 mm pitch, FOV [-128, 128) mm (pixel centers)
    y = np.arange(-128.0, 128.0, 1.0)
    x = np.arange(-128.0, 128.0, 1.0)
    axes = (y, x)
...
def plot_losses(train_losses, val_losses, out_path: Path, title_suffix: str = "") -> None:
...
def dataset_pt_path(dataset_dir: Path, idx: int) -> Path:
    return dataset_dir / f"Art_dataset_coll0_{idx}.pt"


def torch_load_dataset_robust(path: Path) -> v9.CustomDataset:
    """Load a dataset file robustly.

    Handles the common case where the dataset was saved when CustomDataset was
    defined under __main__ (script execution), which otherwise breaks torch.load.
    """
    try:
        return torch.load(path, map_location="cpu")
    except Exception:
        # Provide CustomDataset in __main__ for pickles that reference it.
        sys.modules["__main__"].CustomDataset = v9.CustomDataset
        return torch.load(path, map_location="cpu")
...
def load_or_generate_dataset(
    ...
    path = dataset_pt_path(dataset_dir, idx)
    if path.exists():
        return torch_load_dataset_robust(path)

    if not regenerate_missing:
        raise RuntimeError(f"Dataset {idx} missing at {path}; rerun with --regenerate-missing")

    if KM is None:
        raise RuntimeError("KM kernel required to regenerate dataset")

    # NOTE: v9 generation saves into VMAT_Art_data_256 unconditionally.
    ds = v9.generate_and_save_dataset(idx, KM)
    gen_path = dataset_pt_path(Path("VMAT_Art_data_256"), idx)
    if dataset_dir.resolve() != Path("VMAT_Art_data_256").resolve():
        print(f"[WARN] Dataset regenerated to {gen_path} (v9 default), not {path}")
    return ds


def main() -> None:
    parser = argparse.ArgumentParser(description="Inference + visualization for EPID v9")
    ...
    parser.add_argument("--checkpoint", type=str, default="Cross_CP/EPID_v9_08Jan26_checkpoint.pth")
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument(
        "--regenerate-missing",
        action="store_true",
        dest="regenerate_missing",
        help="Regenerate dataset if not found (uses v9 generated KM)",
    )
    parser.add_argument("--output-dir", type=str, default="viz_outputs_v9", help="Directory for PNGs")
    ...
    dataset_range = args.dataset_range or args.dataset_range
    dataset_indices = list(parse_range(dataset_range))
    if not dataset_indices:
        raise RuntimeError("No dataset indices parsed")
    print(f"[INFO] Using dataset indices: {dataset_indices}")

    dataset_dir = Path("VMAT_Art_data_256")
    print(f"[INFO] Using dataset directory: {dataset_dir}")

    KM = None
    if args.regenerate_missing:
        KM = v9.build_km_kernel_1mm(size=256)
        print("[INFO] Built v9 KM kernel for regeneration")

    datasets = {idx: load_or_generate_dataset(idx, dataset_dir, args.regenerate_missing, KM) for idx in dataset_indices}
    for idx, ds in datasets.items():
        print(f"[INFO] Dataset {idx} loaded with {len(ds)} samples")

    encoderunet = v9.EncoderUNet(vector_dim=104, scalar_count=5).to(device)
    unetdecoder = v9.UNetDecoder(vector_dim=104, scalar_count=5).to(device)
    ...
    losses_png = out_dir / "losses.png"
    if train_losses or val_losses:
        print(f"[INFO] Loss history in checkpoint: train={len(train_losses)} val={len(val_losses)}")
        ...
        plot_losses(
            ...
            val_losses,
            losses_png,
            title_suffix=f"Checkpoint: {ckpt_path.name} | train={len(train_losses)} val={len(val_losses)}",
        )
    else:
        if losses_png.exists():
            try:
                losses_png.unlink()
                print(f"[WARN] Checkpoint has no loss history; deleted stale plot: {losses_png}")
            except OSError:
                print(f"[WARN] Checkpoint has no loss history; stale plot exists: {losses_png}")
        else:
            print("[WARN] Checkpoint has no loss history; skipping loss plot.")
    ...
    for ds_idx, ds in datasets.items():
        ...
        for si in chosen:
            ...
            title = f"EPID v9 | Dataset {ds_idx} | Sample {si}"
            visualize(
                epid_gt,
                epid_img,
                v1_gt,
