...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        output = F.interpolate(output, size=(self.resize_out, self.resize_out), mode='bilinear', align_corners=False)
        return output
...
class UNet2(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.interpolate(x, size=(self.resize_in, self.resize_in), mode='bilinear', align_corners=False)
            ...
            for i, train_loader in enumerate(train_loaders):
                ...
                for batch_idx, (v1, v2, scalars, v1_weight, v2_weight, arrays, arrays_p) in enumerate(train_loader):
                    ...
                    with autocast():
                        ...
                        penalty_diff = v1_reconstructed - v2_reconstructed
                        penalty_mask = (v2_reconstructed < v1_reconstructed)
                        penalty_loss = torch.where(penalty_mask,
                                                 penalty_diff,
                                                 torch.zeros_like(v2_reconstructed)).sum()
                        
                        penalty_diff = v1_reconstructed_2 - v2_reconstructed_2
                        penalty_mask = (v2_reconstructed_2 < v1_reconstructed_2)
                        penalty_loss_2 = torch.where(penalty_mask,
                                                 penalty_diff,
...
def train_ddp(rank, world_size, generate_flag, KM):
    ...
    for dataset_num in range(0, 161,80):    
        start_time = time.time()
